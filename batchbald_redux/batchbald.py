# AUTOGENERATED! DO NOT EDIT! File to edit: 01_batchbald.ipynb (unless otherwise specified). (nbdev)
...
def get_batchbald_batch(
    ...
    N, K, C = log_probs_N_K_C.shape  # N is N datapoints in pool, K is ensemble size (num_inference_samples), C is num classes
    ...
    # H(y1:b|x1:b, D) - a joint entropy, which requires a large number of samples to estimate
...
def get_random_batch(pool_size: int, batch_size: int): -> CandidateBatch:
    return CandidateBatch(torch.ones(batch_size).tolist(), torch.randperm(pool_size)[:batch_size].tolist())


def get_jepig_batch(log_probs_N_K_C: torch.Tensor, batch_size: int, dtype=None, device=None) -> CandidateBatch:
